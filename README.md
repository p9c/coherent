...
The kernel can be written in Go, and if structured so memory management is abstracted, one can allocate a large block of memory to run the memory server, like a container, except, one kernel per virtual core, implemented as one single thread Go application with a pipe to the controlling launcher application that also creates an automatic pipe rendezvous that allows each kernel to message each other kernel.
